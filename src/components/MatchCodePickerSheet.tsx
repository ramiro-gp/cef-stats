...
export function MatchCodePickerSheet({ matches, groups, entries, userId, onSelect, onClose }: { matches: Match[]; groups: Group[]; entries: StatEntry[]; userId: string; onSelect: (selection: MatchLinkSelection) => void; onClose: () => void }) {
  ...
  return <ModalSheet title="Vincular con código o link" onClose={onClose}>
    <input autoFocus value={query} onChange={event => { setQuery(event.target.value); setTeam(null) }} placeholder="CEF-XXXXXXXXXXXX o https://..." className="h-12 w-full rounded-xl border border-slate-200 bg-transparent px-3 font-mono outline-none focus:border-emerald-500 dark:border-white/10" />
    ...
    {match && <div className="mt-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10"><p className="font-extrabold">{match.title}</p><p className="mt-1 text-xs text-slate-400">{new Date(match.scheduledAt).toLocaleDateString('es-AR')} · {match.groupId ? groups.find(group => group.id === match.groupId)?.name ?? 'Grupo anfitrión' : 'Sin grupo'} · {match.format ?? 'F5'}</p><div className="mt-4 grid grid-cols-2 gap-2">{(['light', 'dark'] as MatchTeam[]).map(value => { const full = isTeamFull(match, value, userId); const count = match.participants.filter(participant => participant.team === value).length; const max = getMaxTeamSize(match.format ?? 'F5'); const label = value === 'light' ? match.lightTeamName : match.darkTeamName; return <button key={value} disabled={full} onClick={() => setTeam(value)} className={`min-h-12 truncate rounded-xl px-2 text-sm font-bold disabled:opacity-35 ${team === value ? 'bg-emerald-500 text-ink' : 'border border-slate-200 dark:border-white/10'}`}>{label} {count}/{max}{full ? ' · completo' : ''}</button> })}</div>{duplicate && <p className="mt-3 text-xs font-bold text-amber-500">Ya tenés stats en este partido. Editalas desde su detalle.</p>}</div>}
