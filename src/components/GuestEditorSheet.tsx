...
export function GuestEditorSheet({ guest, team, teamName, onSave, onClose }: { guest?: MatchParticipant; team: MatchTeam; teamName?: string; onSave: (values: { name: string; avatar?: string; team: MatchTeam }) => void | Promise<void>; onClose: () => void }) {
  const [name, setName] = useState(guest?.guestName ?? '')
  const [avatar, setAvatar] = useState(guest?.avatar ?? '')
  const [error, setError] = useState('')
  const [saving, setSaving] = useState(false)
  const save = async () => { if (!name.trim()) { setError('El nombre es obligatorio.'); return } setSaving(true); setError(''); try { await onSave({ name: name.trim(), avatar: avatar.trim(), team }); onClose() } catch (reason) { setError(reason instanceof Error ? reason.message : 'No pudimos guardar al invitado.') } finally { setSaving(false) } }
  return <ModalSheet title={guest ? 'Editar invitado' : `Invitado a ${teamName || (team === 'light' ? 'CLARO' : 'OSCURO')}`} onClose={onClose}>
    ...
    <div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onClose} disabled={saving} className="min-h-12 rounded-xl border border-slate-200 font-bold disabled:opacity-50 dark:border-white/10">Cancelar</button><button onClick={() => void save()} disabled={saving} className="min-h-12 rounded-xl bg-emerald-500 font-bold text-ink disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar'}</button></div>
  </ModalSheet>
}
