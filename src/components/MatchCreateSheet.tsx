...
export function MatchCreateSheet({ onCreate, onClose }: { onCreate: (values: { title: string; scheduledAt: string; format?: MatchFormat }) => Match; onClose: () => void }) {
  ...
  const create = () => { onCreate({ title: title.trim() || 'Partido de hoy', scheduledAt: scheduledAt ? new Date(scheduledAt).toISOString() : new Date().toISOString(), format }); onClose() }
  ...
  return <ModalSheet title="+ Partido" onClose={onClose}>
    ...
    <div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onClose} className="min-h-12 rounded-xl border border-slate-200 font-bold dark:border-white/10">Cancelar</button><button onClick={create} className="min-h-12 rounded-xl bg-emerald-500 font-extrabold text-ink">Crear partido</button></div>
  </ModalSheet>
}
