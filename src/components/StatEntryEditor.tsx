...
interface Props {
  ...
  linkedMatch?: Match
  onSave: (values: Pick<StatEntry, 'result' | 'goals' | 'assists'> & { contextId: string }) => void | Promise<void>
...
export function StatEntryEditor({ entry, groups, personalContextId, linkedMatch, onSave, onDelete, onClose }: Props) {
  ...
  const save = async () => {
    ...
    setError('')
    try {
      await onSave({ result, goals, assists, contextId })
      onClose()
    } catch (reason) {
  ...
  return <ModalSheet title="Editar carga" onClose={onClose}>
    <label className="mb-4 block"><span className="text-xs font-bold uppercase tracking-wider text-slate-500">Contexto de la carga</span><select value={contextId} onChange={event => { setContextId(event.target.value); setError('') }} disabled={Boolean(entry.matchId)} className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-3 font-bold outline-none focus:border-emerald-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-white/10 dark:bg-[#102019]"><option value="">Elegí un contexto</option>{personalContextId && <option value={personalContextId}>Personal (sin grupo)</option>}{groups.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select>{entry.matchId ? <span className="mt-2 block text-xs leading-5 text-amber-600 dark:text-amber-400">Esta carga está vinculada{linkedMatch ? ` a “${linkedMatch.title}”` : ' a un partido'}. Para cambiarla a otro contexto, primero debe desvincularse del partido.</span> : <span className="mt-2 block text-xs leading-5 text-slate-400">Sólo aparecen Personal y los grupos a los que pertenecés.</span>}</label>
    ...
    <div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onClose} disabled={saving} className="min-h-12 rounded-xl border border-slate-200 font-bold disabled:opacity-50 dark:border-white/10">Cancelar</button><button onClick={() => void save()} disabled={saving || !contextId} className="min-h-12 rounded-xl bg-emerald-500 font-extrabold text-ink disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar cambios'}</button></div>
