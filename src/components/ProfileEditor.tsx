  ...
  return <ModalSheet title="Editar perfil" onClose={onClose}>
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-slate-200 p-4 dark:border-white/10">
        <h3 className="text-sm font-extrabold">Preferencias de carga</h3><p className="mt-1 text-xs leading-5 text-slate-400">Elegí los defaults para cargar rápido. Siempre podés cambiarlos antes de guardar.</p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="block"><span className="text-xs font-bold text-slate-500">Tipo de partido</span><select value={defaultMatchType} onChange={event => setDefaultMatchType(event.target.value as LoadMatchTypePreference)} className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-3 outline-none focus:border-emerald-500 dark:border-white/10 dark:bg-[#102019]"><option value="friendly">Amistoso</option><option value="tournament">Torneo</option><option value="ask">Preguntarme</option></select></label>
          <label className="block"><span className="text-xs font-bold text-slate-500">Formato</span><select value={defaultFootballFormat} onChange={event => setDefaultFootballFormat(event.target.value as LoadFormatPreference)} className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-3 outline-none focus:border-emerald-500 dark:border-white/10 dark:bg-[#102019]"><option value="F5">F5</option><option value="F8">F8</option><option value="ask">Preguntarme</option></select></label>
        </div>
      </section>
    </div>
