...
export function ProfileEditor({ user, accountMode = false, onSave, onClose }: { user: User; accountMode?: boolean; onSave: (user: User) => void | string | Promise<void | string>; onClose: () => void }) {
  ...
  return <ModalSheet title="Editar perfil" onClose={onClose}>
    ...
    <div className="space-y-4">
      ...
      <section><div className="flex items-center justify-between gap-3"><span className="text-xs font-bold text-slate-500">Avatar</span><button type="button" onClick={() => setAvatar('')} className="text-xs font-bold text-emerald-500">Usar iniciales</button></div><div className="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-5">{avatarOptions.map(option => <button type="button" key={option.key} onClick={() => setAvatar(option.key)} title={option.label} aria-label={option.label} aria-pressed={avatar === option.key} className={`min-w-0 rounded-2xl border-2 p-1 transition ${avatar === option.key ? 'border-emerald-500 bg-emerald-500/10 ring-2 ring-emerald-500/20' : 'border-slate-200 hover:border-slate-300 dark:border-white/10 dark:hover:border-white/25'}`}><UserAvatar value={option.key} className="aspect-square w-full rounded-xl" /><span className="mt-1 block truncate px-0.5 text-[9px] font-bold text-slate-500 dark:text-slate-300">{option.label}</span></button>)}</div></section>
