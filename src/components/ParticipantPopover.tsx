...
interface Props {
  ...
  isMvp: boolean
  isCreator: boolean
  onSaveGuestStats: (goals: number, assists: number) => void
  onEditGuest: () => void
  onRemoveGuest: () => void
  onClose: () => void
}

export function ParticipantPopover({ participant, anchor, user, entry, guestStats, isMvp, isCreator, onSaveGuestStats, onEditGuest, onRemoveGuest, onClose }: Props) {
  ...
  const name = guest ? participant.guestName : participant.userId === user.id ? user.name : 'Jugador mock'
  const handle = guest ? participant.guestHandle : participant.userId === user.id ? user.username : undefined
  ...
  const save = () => {
    onSaveGuestStats(goals, assists)
    setSaved(true)
  ...
  return <div ref={root} role="dialog" aria-label={`Stats de ${name}`} style={position} className="fixed z-50 rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-2xl dark:border-white/10 dark:bg-[#102019] dark:text-white">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-extrabold">{name}</p>{handle && <p className="truncate text-xs text-slate-400">@{handle.replace(/^@/, '')}</p>}<p className="mt-1 text-[9px] font-black uppercase tracking-wider text-emerald-500">Equipo {participant.team === 'light' ? 'claro' : 'oscuro'} · {guest ? 'Invitado' : 'Usuario'} {isMvp ? '· MVP ⭐' : ''}</p></div><button type="button" onClick={onClose} aria-label="Cerrar" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-slate-100 dark:bg-white/5">×</button></div>
    {guest && isCreator ? <><div className="mt-4 flex gap-4"><MiniCounter label="Goles" value={goals} onChange={value => { setGoals(value); setSaved(false) }} /><MiniCounter label="Asistencias" value={assists} onChange={value => { setAssists(value); setSaved(false) }} /></div><button type="button" onClick={save} className="mt-4 min-h-10 w-full rounded-xl bg-emerald-500 text-sm font-bold text-ink">{saved ? 'Stats guardadas ✓' : 'Guardar stats'}</button><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={onEditGuest} className="min-h-9 text-xs font-bold text-slate-500">Editar nombre</button><button type="button" onClick={onRemoveGuest} className="min-h-9 text-xs font-bold text-rose-500">Quitar</button></div></> : <div className="mt-4 grid grid-cols-2 gap-2 text-center"><div className="rounded-xl bg-slate-100 p-3 dark:bg-white/5"><div className="text-xl font-black">{guest ? guestStats?.goals ?? 0 : entry?.goals ?? 0}</div><div className="text-[9px] text-slate-400">GOLES</div></div><div className="rounded-xl bg-slate-100 p-3 dark:bg-white/5"><div className="text-xl font-black">{guest ? guestStats?.assists ?? 0 : entry?.assists ?? 0}</div><div className="text-[9px] text-slate-400">ASIST.</div></div></div>}
  </div>
}
