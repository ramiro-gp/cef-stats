...
export function ParticipantPopover({ participant, anchor, user, entry, guestStats, isMvp, isCreator, onSaveGuestStats, onEditGuest, onRemoveGuest, onClose }: Props) {
  ...
  return <div ref={root} role="dialog" aria-label={`Stats de ${name}`} style={position} className="fixed z-50 rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-2xl dark:border-white/10 dark:bg-[#102019] dark:text-white">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-extrabold">{name}</p>{handle && <p className="truncate text-xs text-slate-400">@{handle.replace(/^@/, '')}</p>}<p className="mt-1 text-[9px] font-black uppercase tracking-wider text-emerald-500">{participant.team ? `Equipo ${participant.team === 'light' ? 'claro' : 'oscuro'}` : 'Pendiente de equipo'} · {guest ? 'Invitado' : 'Usuario'} {isMvp ? '· MVP ⭐' : ''}</p></div><button type="button" onClick={onClose} aria-label="Cerrar" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-slate-100 dark:bg-white/5">×</button></div>
