...
export function LinkEntrySheet({ entry, matches, groups, allEntries, onLink, onEditExisting, onClose }: Props) {
  ...
  return <ModalSheet title="Vincular a partido" onClose={onClose}>
    <label className="text-xs font-bold text-slate-500">Código o link de invitación</label><input autoFocus value={query} onChange={event => { setQuery(event.target.value); setTeam(null); setConfirmResult(false); setError('') }} placeholder="CEF-XXXXXXXXXXXX o https://..." className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-transparent px-3 font-mono outline-none focus:border-emerald-500 dark:border-white/10" />
    ...
    {match && <div className="mt-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10"><p className="font-extrabold">{match.title}</p><p className="mt-1 text-xs text-slate-400">{new Date(match.scheduledAt).toLocaleDateString('es-AR')} · {groupName} · {match.format ?? 'F5'}</p><div className="mt-4 grid grid-cols-2 gap-2">{(['light', 'dark'] as MatchTeam[]).map(value => { const full = isTeamFull(match, value, entry.userId); const label = value === 'light' ? match.lightTeamName : match.darkTeamName; return <button key={value} disabled={full} onClick={() => { setTeam(value); setConfirmResult(false) }} className={`min-h-12 truncate rounded-xl px-2 text-sm font-bold disabled:opacity-35 ${team === value ? 'bg-emerald-500 text-ink' : 'border border-slate-200 dark:border-white/10'}`}>{label}{full ? ' · completo' : ''}</button> })}</div>{calculated && <p className="mt-3 text-xs text-slate-400">El resultado indica: <strong>{calculated === 'win' ? 'Ganaste' : calculated === 'draw' ? 'Empataste' : 'Perdiste'}</strong>.</p>}{duplicateEntry && <div className="mt-3 rounded-xl bg-amber-500/10 p-3"><p className="text-xs font-bold text-amber-500">Ya existe una carga tuya para este partido.</p><button onClick={() => { onClose(); onEditExisting(duplicateEntry) }} className="mt-2 min-h-9 text-xs font-black text-emerald-500">Editar esa carga →</button></div>}</div>}
