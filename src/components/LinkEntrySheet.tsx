...
interface Props { entry: StatEntry; matches: Match[]; groups: Group[]; allEntries: StatEntry[]; onLink: (entryId: string, matchId: string, team: MatchTeam, result?: MatchResult) => boolean; onEditExisting: (entry: StatEntry) => void; onClose: () => void }
...
export function LinkEntrySheet({ entry, matches, groups, allEntries, onLink, onEditExisting, onClose }: Props) {
  ...
  const link = () => { if (!match || !team) return; if (calculated && calculated !== entry.result && !confirmResult) { setConfirmResult(true); return } if (!onLink(entry.id, match.id, team, calculated ?? undefined)) { setError('No se pudo vincular: revisá cupo o carga duplicada.'); return } onClose() }
  ...
  return <ModalSheet title="Vincular a partido" onClose={onClose}>
    ...
    <div className="mt-6 grid grid-cols-2 gap-3"><button onClick={onClose} className="min-h-12 rounded-xl border border-slate-200 font-bold dark:border-white/10">Cancelar</button><button onClick={link} disabled={!match || !team || Boolean(duplicateEntry)} className="min-h-12 rounded-xl bg-emerald-500 font-bold text-ink disabled:opacity-40">{confirmResult ? 'Confirmar y actualizar' : 'Vincular stats'}</button></div>
  </ModalSheet>
}
