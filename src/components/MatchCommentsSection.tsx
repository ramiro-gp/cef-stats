...
export function MatchCommentsSection({ comments, userId, onSave, onDelete }: Props) {
  ...
  return <section className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] p-4">
    ...
    <div className="mt-5 space-y-3">{sorted.length === 0 && <p className="rounded-xl border border-dashed border-amber-500/20 p-4 text-center text-xs text-slate-400">Todavía nadie picó el partido.</p>}{sorted.map(comment => <article key={comment.id} className="rounded-xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-white/[0.04]"><div className="flex items-center gap-2.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-emerald-500 text-[10px] font-black text-ink">{comment.authorAvatar || initials(comment.authorName, comment.authorHandle)}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{comment.authorName}</p><p className="truncate text-[10px] text-slate-400">@{comment.authorHandle.replace(/^@/, '')} · {commentDate(comment.updatedAt)}{comment.updatedAt !== comment.createdAt ? ' · editado' : ''}</p></div></div><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-6 text-slate-700 dark:text-slate-200">{comment.body}</p></article>)}</div>
  </section>
}
