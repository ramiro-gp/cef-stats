...
export function MatchJoinSheet({ matches, remoteMode = false, initialValue = '', onLookup, onOpen, onClose }: Props) {
  ...
  return <ModalSheet title="Unirme con código" onClose={onClose}>
    ...
    <input autoFocus value={query} onChange={event => { setQuery(formatInviteCodeInput(event.target.value)); setMatch(null); setStatus('idle'); setError('') }} onKeyDown={event => { if (event.key === 'Enter') void search() }} placeholder="H63K-81HY o link de invitación" className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-transparent px-3 font-mono uppercase outline-none focus:border-emerald-500 dark:border-white/10" />
