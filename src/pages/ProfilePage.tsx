...
import { formatEntryDate, resultLabels } from '../utils/format'
...
export function ProfilePage({ user, group, entries, allEntries, matches, groups, totals, worldCup, theme, onSaveUser, onUpdateEntry, onDeleteEntry, onLinkEntry, onTheme, onReset, onLogout, onOpenMatch, accountMode = false, statsError = '' }: Props) {
    ...
    <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
      <div className="space-y-4">
        <section className="relative overflow-hidden rounded-[28px] bg-[#0c2019] p-6 text-white">
          ...
          <div className="relative flex items-center gap-4"><div className="grid h-20 w-20 place-items-center rounded-3xl bg-emerald-500 text-2xl font-black text-ink">{user.avatar}</div><div className="min-w-0"><div className="truncate text-xl font-black">{user.name}</div><div className="mt-1 text-sm text-slate-400">@{user.username.replace(/^@/, '')}</div>{totals.scoringStreak > 0 && <span className="mt-2 inline-block rounded-full bg-emerald-500/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-emerald-400">En racha</span>}</div></div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between"><h2 className="font-extrabold">Últimas cargas</h2><span className="text-xs text-slate-400">{entries.length} {accountMode ? 'en Supabase' : 'locales'}</span></div>
        {statsError && <div className="mb-3 rounded-xl border border-rose-500/20 bg-rose-500/10 p-3 text-sm font-semibold text-rose-500">{statsError}</div>}
