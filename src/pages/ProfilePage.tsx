    ...
    <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
      ...
      <div data-tour="profile-history">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/[0.04]">
          ...
          {!historyLoading && history.map((entry, index) => { const linkedMatch = entry.matchId ? matches.find(match => match.id === entry.matchId) : undefined; const matchGroupName = linkedMatch ? (linkedMatch.groupId ? linkedMatch.groupName ?? groups.find(item => item.id === linkedMatch.groupId)?.name ?? 'Grupo anfitrión' : 'Sin grupo') : undefined; const contextName = entry.deletedGroupName ?? (entry.scopeType === 'personal' || !entry.groupId || entry.groupId.startsWith('personal:') ? 'Personal sin grupo' : groups.find(item => item.id === entry.groupId)?.name ?? linkedMatch?.groupName ?? 'Grupo no disponible'); const resultText = entry.result === 'win' ? 'Gané' : entry.result === 'draw' ? 'Empaté' : 'Perdí'; return <div key={entry.id} className={`flex items-center gap-3 p-4 sm:p-5 ${index !== history.length - 1 ? 'border-b border-slate-100 dark:border-white/5' : ''}`}>
