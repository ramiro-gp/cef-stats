...
export function LoginPage({ configured, loading, authError, onEnterLocal, onSignIn, onSignUp }: Props) {
  ...
  const submit = async () => {
    ...
    const result = mode === 'register'
      ? await onSignUp({ email, password, name, handle: normalizeHandle(handle), avatar })
  ...
  const accountForm = mode !== 'welcome' && <div className="my-auto py-8">
    ...
    <p className="mt-2 text-sm leading-6 text-slate-400">Tus stats y partidos siguen guardándose localmente en esta etapa.</p>
    ...
    <div className="mt-6 space-y-4">
      {mode === 'register' && <>
        ...
        <label className="block"><span className="text-xs font-bold text-slate-400">Avatar opcional</span><input value={avatar} maxLength={4} onChange={event => setAvatar(event.target.value)} placeholder="RA o ⚽" className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-white/5 px-3 outline-none focus:border-emerald-500" /></label>
      </>}
    ...
    <div className="relative mx-auto flex w-full max-w-md flex-col">
      ...
      {accountForm || <>
        <div className="my-auto py-14">
          ...
          <p className="mt-5 max-w-sm text-lg leading-7 text-slate-400">Entrá con cuenta o mantené intacto tu <span className="text-white">modo local.</span></p>
        ...
        <div className="space-y-3"><button type="button" onClick={() => setMode('login')} className="min-h-14 w-full rounded-2xl bg-emerald-500 px-6 font-extrabold text-ink">Entrar con cuenta</button><button type="button" onClick={onEnterLocal} className="min-h-14 w-full rounded-2xl border border-white/10 bg-white/5 px-6 font-extrabold">Seguir en modo local</button></div>
        <p className="mt-4 text-center text-xs text-slate-600">El modo local conserva todos tus datos actuales</p>
      </>}
    </div>
  </div>
}
