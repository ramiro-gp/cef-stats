...
export function RankingsPage({ group, players, sourceEntries, sourceUsers, currentUserId }: Props) {
  ...
  return <>
    ...
    <div className="mb-5 grid grid-cols-2 rounded-2xl bg-slate-100 p-1 dark:bg-white/5"><button type="button" onClick={() => setView('ranking')} className={`min-h-11 rounded-xl text-sm font-extrabold transition ${view === 'ranking' ? 'bg-white text-slate-900 shadow-sm dark:bg-white/10 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>Ranking</button><button type="button" onClick={() => setView('statistics')} className={`min-h-11 rounded-xl text-sm font-extrabold transition ${view === 'statistics' ? 'bg-white text-slate-900 shadow-sm dark:bg-white/10 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>Estadísticas</button></div>
