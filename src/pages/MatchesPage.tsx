...
import { getMatchStatTotals, getMaxTeamSize, getParticipantAvatar, getParticipantName, isTeamFull } from '../utils/matches'
...
export function MatchesPage({ group, user, matches, entries, initialMatchId = null, initialInviteCode = '', remoteMode = false, loading = false, loadError = '', onLookupMatch, onInviteConsumed, onCreate, onJoinTeam, onLeave, onScore, onMvp, onSaveStats, onAddGuest, onUpdateGuest, onRemoveGuest, onSaveGuestStats }: Props) {
  ...
  return <>
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
      ...
      <aside className="space-y-5">
        ...
        {isCreator && selected.participants.length > 0 && <section className="rounded-2xl border border-violet-500/20 bg-violet-500/[0.06] p-4"><div className="flex items-center gap-2"><TrophyIcon className="h-5 w-5 text-violet-500"/><h3 className="font-extrabold">MVP manual</h3></div><div className="mt-3 space-y-2">{selected.participants.map(participant => <button key={participant.id} onClick={() => { void Promise.resolve(onMvp(selected.id, participant.id)).catch(() => undefined) }} className={`flex min-h-11 w-full items-center justify-between rounded-xl px-3 text-sm font-bold ${selected.mvpParticipantId === participant.id ? 'bg-violet-500 text-white' : 'border border-violet-500/20'}`}><span>{playerName(participant)}</span>{selected.mvpParticipantId === participant.id && <CheckIcon className="h-4 w-4" />}</button>)}</div></section>}
      </aside>
    </div>
    ...
    {editingGuest && <GuestEditorSheet guest={editingGuest} team={editingGuest.team} onSave={async values => { await onUpdateGuest(selected.id, editingGuest.id, values) }} onClose={() => setEditingGuest(null)} />}
