...
export function MatchesPage({ group, user, matches, entries, initialMatchId = null, initialInviteCode = '', remoteMode = false, loading = false, loadError = '', creationGroups = [], onLookupMatch, onInviteConsumed, onOpenMatch, onCloseMatch, onCreate, onAttend, onOmit, onJoinTeam, onParticipantTeam, onLeave, onScore, onMvp, onSaveComment, onDeleteComment, onSaveStats, onAddGuest, onUpdateGuest, onRemoveGuest, onSaveGuestStats, onUndoableAction }: Props) {
  ...
  if (!selected) return <>
    ...
    {!loading && (matches.length === 0 ? <div className="rounded-2xl border border-dashed border-emerald-500/30 bg-emerald-500/[0.05] p-8 text-center"><div className="text-3xl">⚽</div><p className="mt-3 font-extrabold">Todavía no tenés partidos en este scope</p><p className="mt-1 text-sm text-slate-400">{allowCreate ? 'Creá uno o ingresá un código de invitación.' : 'Ingresá un código de invitación para abrir un partido.'}</p>{allowCreate && <button onClick={() => setCreating(true)} className="mt-5 min-h-12 rounded-xl bg-emerald-500 px-5 font-bold text-ink">Crear primer partido</button>}</div> : <>
      <div className="grid gap-3 sm:grid-cols-2">{paginatedMatches.map(match => {
        ...
        return <article key={match.id} className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-emerald-500/40 dark:border-white/10 dark:bg-white/[0.04]">
          ...
          {mine && <button type="button" onClick={() => setLeaveTarget(match)} className="mt-4 min-h-10 w-full rounded-xl border border-rose-500/20 text-xs font-bold text-rose-500 transition hover:bg-rose-500/10">Salir del partido</button>}
        </article>
      })}</div>
