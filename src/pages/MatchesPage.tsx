...
export function MatchesPage({ group, user, matches, entries, initialMatchId = null, initialInviteCode = '', remoteMode = false, loading = false, loadError = '', creationGroups = [], onLookupMatch, onInviteConsumed, onOpenMatch, onCloseMatch, onCreate, onUpdateMatch, onAttend, onOmit, onJoinTeam, onParticipantTeam, onLeave, onScore, onMvp, onSaveComment, onDeleteComment, onSaveStats, onAddGuest, onUpdateGuest, onRemoveGuest, onSaveGuestStats, onUndoableAction }: Props) {
  ...
  const scopeSubtitle = isAllScope(group)
    ? 'Todos los partidos donde participás.'
    : isPersonalScope(group)
      ? 'Partidos sin grupo donde participás.'
      : `Partidos de ${group.name} donde participás.`
  ...
  if (!selected) return <>
    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><PageTitle eyebrow={group.name} title="Partidos" subtitle={remoteMode ? scopeSubtitle : allowCreate ? 'Organizá la próxima fecha sin tapar la carga rápida.' : 'Elegí un grupo real para crear partidos, o ingresá con un código.'} /><div className="flex shrink-0 gap-2"><button data-tour="matches-join" onClick={() => setJoiningByCode(true)} className="min-h-12 rounded-xl border border-emerald-500/30 px-4 text-sm font-extrabold text-emerald-500">Unirme con código</button>{allowCreate && <button data-tour="matches-create" onClick={() => setCreating(true)} className="min-h-12 rounded-xl bg-emerald-500 px-4 text-sm font-extrabold text-ink">+ Partido</button>}</div></div>
    ...
    <div data-tour="matches-heading" className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">Partidos según el grupo que elegiste ver</div>
    ...
    {!loading && (matches.length === 0 ? <div className="rounded-2xl border border-dashed border-emerald-500/30 bg-emerald-500/[0.05] p-8 text-center"><div className="text-3xl">⚽</div><p className="mt-3 font-extrabold">Todavía no tenés partidos en esta vista</p><p className="mt-1 text-sm text-slate-400">{allowCreate ? 'Creá uno o ingresá un código de invitación.' : 'Ingresá un código de invitación para abrir un partido.'}</p>{allowCreate && <button onClick={() => setCreating(true)} className="mt-5 min-h-12 rounded-xl bg-emerald-500 px-5 font-bold text-ink">Crear primer partido</button>}</div> : <>
      <div className="grid gap-3 sm:grid-cols-2">{paginatedMatches.map(match => {
        ...
        return <article key={match.id} className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-emerald-500/40 dark:border-white/10 dark:bg-white/[0.04]">
          <button type="button" onClick={() => { setActiveParticipant(null); openMatch(match.id) }} className="w-full text-left">
            ...
            <div className="mt-4 flex items-center justify-between gap-3 text-sm"><span className="text-slate-400">{match.format ?? 'Formato libre'} · {match.participants.length} anotados</span><span className="text-right">{pendingTeam(match) && <span className="block text-[10px] font-bold text-amber-500">Pendiente de equipo</span>}<span className="font-black">{match.score ? `${match.score.light}–${match.score.dark}` : 'Ver →'}</span></span></div>
          </button>
  ...
  return <>
    ...
    {!myParticipant && <section className="mb-5 rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.08] p-4"><p className="font-extrabold text-emerald-600 dark:text-emerald-400">¿Jugás este partido?</p><p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-300">Unite para elegir equipo y cargar tus números vinculados automáticamente.</p><button type="button" onClick={() => void attend(selected)} disabled={Boolean(pendingAction)} className="mt-4 min-h-12 w-full rounded-xl bg-emerald-500 font-black text-ink disabled:opacity-50">UNIRME AL PARTIDO</button></section>}
    <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
      ...
      <aside className="space-y-5">
        {myParticipant?.team && <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.06] p-4"><h3 className="font-extrabold">Mis stats</h3><p className="mt-1 text-xs leading-5 text-slate-400">{myEntry ? `${resultLabels[myEntry.result]} · ${myEntry.goals} goles · ${myEntry.assists} asistencias` : 'Todavía no cargaste stats para este partido.'}</p><button onClick={() => setStatsOpen(true)} className="mt-4 min-h-12 w-full rounded-xl bg-emerald-500 px-3 text-sm font-extrabold text-ink">{myEntry ? 'Editar stats de este partido' : 'Cargar stats de este partido'}</button></section>}
