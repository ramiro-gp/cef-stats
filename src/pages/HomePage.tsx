...
export function HomePage({ user, group, entries, matches, matchEvents, totals, rankings, worldCup, onNavigate }: Props) {
  const personalScope = isPersonalScope(group)
  const feed = buildActivityFeed(entries, user, group, matches, matchEvents)
  ...
  return <>
    <section className="relative overflow-hidden rounded-[28px] bg-[#0c2019] p-5 text-white shadow-xl dark:border dark:border-white/5 sm:p-7">
      ...
      <div className="relative">
        ...
        <div className="mt-2 flex items-end justify-between gap-4">
          <div><h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">Buenas, {user.nickname} 👋</h1><p className="mt-1 text-sm text-slate-400">{personalScope ? 'Estás cargando en Mi historial.' : '¿Cómo te fue en la cancha?'}</p></div>
          <div className="hidden rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-right sm:block"><div className="text-xs text-slate-400">Cargas locales</div><div className="text-sm font-bold">{entries.length} partido{entries.length === 1 ? '' : 's'}</div></div>
        </div>
    ...
    <section className="mt-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          { label: 'Mis goles', value: String(totals.goals), detail: personalScope ? 'Historial personal' : hasRankings ? `#${goalPosition} del grupo` : 'Sin ranking todavía', icon: '⚽', color: 'text-emerald-500' },
          { label: 'Mis asistencias', value: String(totals.assists), detail: personalScope ? 'Historial personal' : hasRankings ? `#${assistPosition} del grupo` : 'Sin ranking todavía', icon: '→', color: 'text-sky-500' },
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[1.4fr_.8fr]">
      <section>
        <div className="mb-3 flex items-center justify-between"><div><h2 className="font-extrabold">Movimientos recientes</h2><p className="mt-0.5 text-xs text-slate-400">Últimos {Math.min(10, feed.length)} de {group.name}</p></div><span className="h-2 w-2 rounded-full bg-emerald-500" /></div>
