...
export default function App() {
  ...
  const personalSeasonEntries = [...new Map((accountMode ? profileHistory.seasonEntries : store.entries.filter(entry => entry.userId === currentUser.id)).map(entry => [entry.id, entry])).values()]
  const allMatches = accountMode ? remoteMatches.matches : store.matches.filter(match => match.groupId === group.id)
  const groupMatches = accountMode ? allMatches.filter(match => !match.groupId || (allScope ? sharedGroupIds.includes(match.groupId) : match.groupId === group.id)) : allMatches
  ...
  const personalSeasonTotals = calculateUserTotals(personalSeasonEntries, false)
  ...
  const personalSeasonWorldCup = calculatePersonalWorldCup(personalSeasonEntries)
  ...
  const pageContent = {
    home: <HomePage user={currentUser} group={group} entries={allScope ? scopeEntries : groupEntries} matches={groupMatches} matchEvents={groupMatchEvents} totals={totals} seasonTotals={personalSeasonTotals} rankings={rankings} worldCup={worldCup} seasonWorldCup={personalSeasonWorldCup} onNavigate={navigate} userNames={userNames} groupNames={groupNames} playerGroupIds={playerGroupIds} />,
