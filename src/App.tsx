...
export default function App() {
  ...
  const pageContent = {
    ...
    add: <AddStatsPage key={group.id} onSave={saveQuickStats} onNavigate={navigate} onNotify={notify} matches={accountMode ? allMatches : groupMatches} groups={accountMode ? remoteGroups.groups : store.groups} entries={accountMode ? scopeEntries : store.entries} user={currentUser} onAttendMatch={accountMode ? remoteMatches.attendMatch : store.attendMatch} onOmitMatch={accountMode ? remoteMatches.omitMatch : store.omitMatch} defaultContextId={accountMode ? remoteGroups.activeSharedGroup?.id ?? (personalScope ? remoteGroups.personalScope?.id ?? '' : '') : group.id} personalContextId={accountMode ? remoteGroups.personalScope?.id : undefined} />,
